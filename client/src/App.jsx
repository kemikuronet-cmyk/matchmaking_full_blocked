...
function App() {
  ...
  useEffect(() => {
    ...
    socket.on("lottery_winner", ({ title }) => {
      setLotteryWinnerTitles((prev) => (!prev.includes(title) ? [...prev, title] : prev));
    });

    socket.on("update_lottery_list", ({ list }) => { if (Array.isArray(list)) { setLotteryList(list); setShowLottery(true); } });
  ...
  // --- localStorage永続化 ---
  useEffect(() => { localStorage.setItem("history", JSON.stringify(history)); socket.emit("history_update", { sessionId: user?.sessionId, history }); }, [history]);
  useEffect(() => { localStorage.setItem("lotteryWinnerTitles", JSON.stringify(lotteryWinnerTitles)); }, [lotteryWinnerTitles]);
  useEffect(() => { localStorage.setItem("lotteryHistory", JSON.stringify(lotteryHistory)); }, [lotteryHistory]);
  ...
  // --- 操作ハンドラ ---
  const handleLogin = () => { const trimmedName = name.trim(); if (!trimmedName) return alert("ユーザー名を入力してください"); const saved = JSON.parse(localStorage.getItem("user") || "{}"); const sessionId = saved?.sessionId || undefined; const recentOpponents = saved?.recentOpponents || []; socket.emit("login", { name: trimmedName, sessionId, history, recentOpponents }); };
  ...
  const handleLogout = () => { if (!window.confirm("ログアウトしますか？")) return; socket.emit("logout"); localStorage.removeItem("user"); localStorage.removeItem("adminMode"); localStorage.removeItem("lotteryWinnerTitles"); localStorage.removeItem("lotteryHistory"); localStorage.removeItem("history"); setUser(null); setLoggedIn(false); setSearching(false); setOpponent(null); setDeskNum(null); setLotteryWinnerTitles([]); setLotteryHistory([]); setHistory([]); setName(""); };
  ...
  const handleDeleteLotteryEntry = (index) => { const entry = lotteryHistory[index]; if (!entry) return; if (!window.confirm(`抽選「${entry.title}」の履歴を削除しますか？`)) return; setLotteryHistory((prev) => { const next = [...prev]; next.splice(index,1); localStorage.setItem("lotteryHistory", JSON.stringify(next)); return next; }); socket.emit("admin_delete_lottery_history", { title: entry.title, index }); };
  const handleClearLotteryHistory = () => { if (!window.confirm("抽選履歴をすべて削除しますか？")) return; setLotteryHistory([]); localStorage.removeItem("lotteryHistory"); socket.emit("admin_clear_lottery_history"); };
