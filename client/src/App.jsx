...
function App() {
  ...
  const [autoLogoutHours, setAutoLogoutHours] = useState(0);
  ...
  useEffect(() => {
    ...
    socket.on("opponent_found", ({ opponent, deskNum }) => {
      setOpponent(opponent);
      setDeskNum(deskNum);
      setSearching(false);
    });

    socket.on("match_enabled", (enabled) => setMatchEnabled(enabled));
    socket.on("lottery_result", (result) => setDrawResult(result));
    socket.on("lottery_history", (history) => setLotteryHistory(history));
    socket.on("user_list", (list) => setUsersList(list));
    socket.on("active_matches", (matches) => setActiveMatches(matches));
    socket.on("update_history", (history) => setHistory(history));
    socket.on("lottery_list", (list) => setLotteryList(list));
    socket.on("lottery_winner_titles", (titles) => setLotteryWinnerTitles(titles));

    return () => {
      socket.off("login_success");
      socket.off("opponent_found");
      socket.off("match_enabled");
      socket.off("lottery_result");
      socket.off("lottery_history");
      socket.off("user_list");
      socket.off("active_matches");
      socket.off("update_history");
      socket.off("lottery_list");
      socket.off("lottery_winner_titles");
    };
  }, []);
  ...
  const handleWinReport = () => {
    socket.emit("report_win", { opponentId: opponent.id, deskNum });
    setOpponent(null);
  };

  // --- 管理者操作系ハンドラ ---
  const handleAdminLogin = () => {
    socket.emit("admin_login", { password: adminPassword }, (success) => {
      if (success) setAdminMode(true);
      else alert("管理者パスワードが違います");
    });
  };
  const handleAdminLogout = () => setAdminMode(false);
  const handleToggleMatch = () => socket.emit("admin_toggle_match");
  const handleDrawLots = () =>
    socket.emit("admin_draw_lots", { count: drawCount, minMatches, minLoginHours });
  const handleUpdateAutoLogout = () =>
    socket.emit("admin_update_auto_logout", { hours: autoLogoutHours });
  const handleLogoutUser = (id, name) =>
    socket.emit("admin_logout_user", { id, name });
  const handleAdminLogoutAll = () => socket.emit("admin_logout_all");

  const handleAdminReportWin = (playerId, deskNum) =>
    socket.emit("admin_report_win", { playerId, deskNum });
      ...
      <div className="app">
        ...
        <div className="admin-screen">
          ...
          <div className="admin-section">
            ...
            {lotteryHistory.length === 0 ? (
              ...
              <table style={{ color: "white", borderCollapse: "collapse" }}>
                <thead><tr><th>抽選名</th><th>当選者</th></tr></thead>
                <tbody>
                  {lotteryHistory.map((l, idx) => (
                    <tr key={idx}>
                      <td>{l.title}</td>
                      <td>{(Array.isArray(l.winners) ? l.winners : []).map((w,i) => (
          ...
          <div className="admin-section">
            <h3>ログイン中のユーザー</h3>
            <table style={{ color: "white", borderCollapse: "collapse" }}>
              <thead><tr><th>番号</th><th>名前</th><th>対戦数</th><th>勝</th><th>敗</th><th>ログイン時間</th><th>操作</th></tr></thead>
              <tbody>
          ...
          <div className="admin-section">
            ...
            {activeMatches.length === 0 ? (
              ...
              <table style={{ color: "white", borderCollapse: "collapse" }}>
                <thead><tr><th>卓番号</th><th>プレイヤー1</th><th>プレイヤー2</th><th>操作</th></tr></thead>
                <tbody>
      ...
      <div className="menu-screen">
        ...
        <div style={{ marginTop: lotteryList.length > 0 ? "15px" : "0px" }}>
          <div className="history-list">
            <h4>対戦履歴</h4>
            <table>
              <thead><tr><th>No.</th><th>対戦相手</th><th>結果</th><th>日時</th></tr></thead>
              <tbody>
                {displayHistory.map((h,i) => (
                  <tr key={i}>
                    <td>{i+1}</td>
                    <td>{h.opponent}</td>
                    <td className={h.result?.toUpperCase() === "WIN" ? "win" : h.result?.toUpperCase() === "LOSE" ? "lose" : ""}>
                      {h.result}
                    </td>
