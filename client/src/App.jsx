...
function App() {
  ...
  // --- keepAlive 送信（セッション維持） ---
  useEffect(() => {
    const interval = setInterval(() => {
      if (loggedIn) {
        const now = Date.now();
        // 5分おきに送信
        if (now - lastKeepAlive.current > 300000) {
          socket.emit("keep_alive", { sessionId: user?.sessionId });
  ...
  const handleWinReport = () => { 
    if (!loggedIn) return alert("セッションが切れています。再ログインしてください");
