...
function loadData() {
  ...
  try {
    ...
    if (data.users) users = data.users.map(u => ({ ...u, recentOpponents: u.recentOpponents || [], id: u.id || null, disconnectedAt: null }));
    if (data.desks) {
...
io.on("connection", socket => {
  ...
  socket.on("admin_draw_lots", ({ count, minBattles, minLoginMinutes, title }) => {
    ...
    // 当選者個別通知（クライアント側で赤字表示）
    selected.forEach(u => {
      if (u.id) io.to(u.id).emit("lottery_winner", { title });
    });
