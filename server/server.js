...
let adminPassword = "admin1234"; // 任意で変更可能
...
const broadcastActiveMatches = () => {
  const active = Object.keys(desks).map((deskNum) => {
    ...
    return {
      ...
      player1SessionId: d.p1?.id,
      player2SessionId: d.p2?.id,
    };
  });
  io.emit("admin_active_matches", active);
};
...
io.on("connection", (socket) => {
  ...
  socket.on("report_win_request", () => {
    ...
    const opponent =
      match.p1.id === socket.id ? match.p2 : match.p1;
  ...
  socket.on("opponent_win_confirmed", ({ accepted }) => {
    ...
    const reporter =
      match.p1.id === match.reported ? match.p1 : match.p2;
    const loser =
      match.p1.id === match.reported ? match.p2 : match.p1;
  ...
  socket.on("admin_report_win", ({ winnerSessionId, deskNum }) => {
    ...
    if (!match) return;
    const winner =
      match.p1.id === winnerSessionId ? match.p1 : match.p2;
    const loser =
      match.p1.id === winnerSessionId ? match.p2 : match.p1;
  ...
  socket.on("admin_logout_user", ({ userId }) => {
    ...
    if (target) {
      io.to(userId).emit("force_logout", { reason: "admin" });
